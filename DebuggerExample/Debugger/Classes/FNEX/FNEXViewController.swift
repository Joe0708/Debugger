...
class FNEXViewController: UIViewController {
    ...
    var currentState: ToolType = .select
    ...
    override func viewDidLoad() {
        ...
        superViewFrameView = UIView(frame: CGRect.zero)
        superViewFrameView?.layer.borderColor = UIColor(red: 0, green: 1, blue: 0, alpha: 1).cgColor
        superViewFrameView?.layer.borderWidth = 1
        superViewFrameView?.backgroundColor = UIColor(red: 0, green: 1, blue: 0, alpha: 0.3)
        view.addSubview(superViewFrameView!)
        
        highLightHorizonalEdgeLine0 = UIView(frame: CGRect(x: 0, y: -1, width: view.frame.width, height: 0.5))
        highLightHorizonalEdgeLine0?.backgroundColor = .red
        view.addSubview(highLightHorizonalEdgeLine0!)
        highLightHorizonalEdgeLine1 = UIView(frame: CGRect(x: 0, y: -1, width: view.frame.width, height: 0.5))
        highLightHorizonalEdgeLine1?.backgroundColor = .red
        view.addSubview(highLightHorizonalEdgeLine1!)
        highLightVerticalEdgeLine0 = UIView(frame: CGRect(x: -1, y: 0, width: 0.5, height: view.frame.height))
        highLightVerticalEdgeLine0?.backgroundColor = .red
        view.addSubview(highLightVerticalEdgeLine0!)
        highLightVerticalEdgeLine1 = UIView(frame: CGRect(x: -1, y: 0, width: 0.5, height: view.frame.height))
        highLightVerticalEdgeLine1?.backgroundColor = .red
        view.addSubview(highLightVerticalEdgeLine1!)
        
        highLightFrameView = UIView(frame: CGRect.zero)
        highLightFrameView?.layer.borderColor = UIColor(red: 1, green: 0, blue: 0, alpha: 1).cgColor
        highLightFrameView?.layer.borderWidth = 1
        highLightFrameView?.backgroundColor = UIColor(red: 1, green: 0, blue: 0, alpha: 0.3)
        ...
        view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(handleTap(tap:))))
        
        toolBar = FNEXToolBar(frame: CGRect(x: 0,
                                            y: 64,
                                            width: view.frame.width,
                                            height: 84),
                              iconArray: [UIImage(named: "cursor")!,
                                          UIImage(named: "collect")!,
                                          UIImage(named: "flow")!,
                                          UIImage(named: "close")!])
        view.addSubview(toolBar!)
        fnexToolBarClickedBlock = { [unowned self] type in
    
            switch type {
            case .menu:
                let vc = UINavigationController(rootViewController: ToolboxViewController())
            ...
            case .tree:
                if nil == self.selectedView { break }
